    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.spuId) {
            this.$http({
              url: this.$http.adornUrl(`/gulimall_product/spuinfodesc/info/${this.dataForm.spuId}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimall_product/spuinfodesc/${!this.dataForm.spuId ? 'save' : 'update'}`),
              method: 'post',
